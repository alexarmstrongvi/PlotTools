...
plot_defaults = {
    ...
    'l_eta[1]'                : Plot1D( bin_range=[-3.0, 3.0],   bin_width=0.5, xlabel='#eta^{Subleading lepton}'),
    ...
    'lep_z0SinTheta[0]'    : Plot1D( bin_range=[-1, 1],     bin_width=0.05, add_underflow=True, doNorm=True, doLogY=False, xunits='mm', xlabel='Lep0 z_{0}sin(#theta)'),
    ...
    'lep_z0SinTheta[2]'    : Plot1D( bin_range=[-1, 1],     bin_width=0.05, add_underflow=True, doNorm=True, doLogY=False, xunits='mm', xlabel='Lep2 z_{0}sin(#theta)'),
    ...
    'l_etconetopo20[1]'    : Plot1D( bin_range=[-2, 5],   nbins=100, add_underflow=True, xunits='GeV', xlabel='Iso E_{T}^{l_{1}} topocone'),
    'l_ptvarcone[1]'       : Plot1D( bin_range=[0.5, 5],     nbins=25, xunits='GeV', xlabel='Iso p_{T}^{l_{1}} varcone'),
    'l_pt[1]+l_etconetopo20[1]'     : Plot1D( bin_range=[0.0, 150.0],  bin_width=5, xunits='GeV', xlabel='p_{T}^{l_{1}}+etconetopo'),
    'l_pt[1]+l_ptvarcone[1]'        : Plot1D( bin_range=[0.0, 150.0],  bin_width=5, xunits='GeV', xlabel='p_{T}^{l_{1}}+ptvarcone'),
    ...
    'l_pt[0]'              : Plot1D( bin_range=[0.0, 150.0],  bin_width=5, xunits='GeV', xlabel='p_{T}^{leading lep}'),
    'l_pt[1]'              : Plot1D( bin_range=[0.0, 100.0],   bin_width=5, xunits='GeV', xlabel='p_{T}^{subleading lep}'),
    ...
    'MLep0'                : Plot1D( bin_range=[0.0, 100],     nbins=25, xunits='GeV', xlabel='M_{l0}'),
    'MLep1'                : Plot1D( bin_range=[0.0, 100],     nbins=25, xunits='GeV', xlabel='M_{l1}'),
    ...
    'drll'                 : Plot1D( bin_range=[0.0, 6.0],    bin_width=0.3, doLogY=False, xlabel='#DeltaR(l_{0},l_{1})'),
    ...
    'el1pT_trackclus_ratio': Plot1D( bin_range=[0, 3],        bin_width=0.1, xlabel='el_{subleading pT}^{track} / el_{subleading pT}^{cluster}', xcut_is_max=True),
    # MET + leptons
    ...
    'MCollASym'            : Plot1D( bin_range=[0.0, 300.0], bin_width=10, doNorm=False, xunits='GeV', xlabel='LFV Collinear Mass m_{coll}'),
    'dpt_ll'               : Plot1D( bin_range=[0, 50.0],  bin_width=1.0, xunits='GeV', xlabel='#Deltap_{T}^{ll}', xcut_is_max=False),
    'DphiLep0MET'          : Plot1D( bin_range=[-0.2, 3.4], bin_width=0.2, add_underflow=True, xlabel='#Delta#phi(l_{0},MET)', xcut_is_max=False),
    'DphiLep1MET'          : Plot1D( bin_range=[-0.2, 3.4], bin_width=0.2, add_underflow=True, xlabel='#Delta#phi(l_{1},MET)', xcut_is_max=False),
    'DphiLep2MET'          : Plot1D( bin_range=[-0.2, 3.4], bin_width=0.2, add_underflow=True, xlabel='#Delta#phi(l_{2},MET)'),
    'tau_pT'               : Plot1D( bin_range=[0.0, 200.0],  nbins=20, xunits='GeV', xlabel='p_{T}^{subleading lep + MET}'),
    'taulep1_pT_ratio'     : Plot1D( bin_range=[0.0, 3],      nbins=20, xlabel='p_{T}^{subleading lep + MET} / p_{T}^{leading lep}', xcut_is_max=False),
    # Jets
    ...
    'mjj'                  : Plot1D( bin_range=[0.0, 300],     nbins=25, xunits='GeV', xlabel='Dijet mass'),
    ...
    'DphiLepJet[2]'        : Plot1D( bin_range=[0.0, 6.0],    bin_width=0.5, xlabel='#DeltaR(l_{2}, closest jet)'),
    ...
    'l_pt[2]'              : Plot1D( bin_range=[0.0, 100.0],   bin_width=5, xunits='GeV', xlabel='p_{T}^{fake lep candidate}'),
    'lep_met_pT[2]'        : Plot1D( bin_range=[0.0, 150.0],  bin_width=5, xunits='GeV', xlabel='p_{T}(fake lepton + MET)'),
    'l_eta[2]'             : Plot1D( bin_range=[-3.0, 3.0],   bin_width=0.5, xlabel='Fake candidate lepton #eta'),
    ...
    'l_mT[2]'              : Plot1D( bin_range=[0.0, 200.0],  bin_width=5, xunits='GeV', xlabel='m_{T}^{l_{2}}'),
    'l_mT[1]'              : Plot1D( bin_range=[0.0, 200.0],  bin_width=5, xunits='GeV', xlabel='m_{T}^{l_{1}}'),
    'l_mT[0]'              : Plot1D( bin_range=[0.0, 200.0],  bin_width=5, xunits='GeV', xlabel='m_{T}^{l_{0}}'),
    'dR_ZLep0_Fake'        : Plot1D( bin_range=[0.0, 6.0],    bin_width=0.3, xlabel='#DeltaR(l_{2}, l_{0})'),
    'DphiL2L[1]'           : Plot1D( bin_range=[0.0, 3.2],    bin_width=0.2, xlabel='#Delta#phi(l_{2}, l_{1})'),
    'DphiL2L[0]'           : Plot1D( bin_range=[0.0, 3.2],    bin_width=0.2, xlabel='#Delta#phi(l_{2}, l_{0})'),
    'drl2l[0]'             : Plot1D( bin_range=[0.0, 6.0],    bin_width=0.3, xlabel='#DeltaR(l_{2}, l_{0})'),
    'drl2l[1]'             : Plot1D( bin_range=[0.0, 6.0],    bin_width=0.3, xlabel='#DeltaR(l_{2}, l_{1})'),
    'dR_ZLep1_Fake'        : Plot1D( bin_range=[0.0, 6.0],    bin_width=0.3, xlabel='#DeltaR(l_{2}, l_{1})'),
    'dR_Z_Fake'            : Plot1D( bin_range=[0.0, 6.0],    bin_width=0.3, xlabel='#DeltaR(l_{2}, Z)'),
    ...
    'DphiLep0MET:drll'     : Plot2D( bin_range = [0, 6, 0.0, 3.2], xbin_width=0.1, ybin_width=0.2, xlabel="#DeltaR(l_{0},l_{1})", ylabel='#Delta#phi(l0,MET)'),
    'DphiLep1MET:drll'     : Plot2D( bin_range = [0, 6, 0.0, 3.2], xbin_width=0.1, ybin_width=0.2, xlabel="#DeltaR(l_{0},l_{1})", ylabel='#Delta#phi(l1,MET)'),
    'DphiLep0MET:l_mT[0]'  : Plot2D( bin_range = [0, 100, 0.0, 3.2], xbin_width=5, ybin_width=0.2, xunits="GeV", xlabel="m_{T}^{l_{0}}", ylabel='#Delta#phi(l0,MET)'),
    'DphiLep1MET:l_mT[1]'  : Plot2D( bin_range = [0, 100, 0.0, 3.2], xbin_width=5, ybin_width=0.2, xunits="GeV", xlabel="m_{T}^{l_{1}}", ylabel='#Delta#phi(l1,MET)'),
    ...
    'l_pt[0]:drll'        : Plot2D( bin_range = [0.0, 3.2, 0, 100], xbin_width=0.2, ybin_width=1, xlabel="#DeltaR(l_{0},l_{1})", ylabel='p_{T}^{l_{0}}'),
    'l_pt[0]:DphiLep1MET' : Plot2D( bin_range = [0.0, 3.2, 0, 100], xbin_width=0.2, ybin_width=1, xunits="GeV", xlabel="#Delta#phi(l1,MET)", ylabel='p_{T}^{l_{0}}'),
    'dpt_ll:drll'         : Plot2D( bin_range = [1.5, 4, 0, 50], xbin_width=0.1, ybin_width=1, xlabel="#DeltaR(l_{0},l_{1})", ylabel='#Deltap_{T}^{ll}'),
...
region_plots['zjets_FF_CR'] = {
  ...
  'MET' : deepcopy(plot_defaults['MET']),
  'l_mT[2]' : deepcopy(plot_defaults['l_mT[2]'])
}
region_plots['zjets_FF_CR']['MLL'].update(bin_range = [75, 105], bin_width=2.5)
region_plots['zjets_FF_CR']['MET'].update(bin_range = [0, 100], bin_width=5)
region_plots['zjets_FF_CR']['l_mT[2]'].update(bin_range = [0, 50], bin_width=5)
